...
'</button>'+
  ...
  $.fn.SetEditable = function (options) {
    ...
    this.find('thead tr').append('<th name="buttons"></th>');  //encabezado vacÃ­o
    this.find('tbody tr').append(colEdicHtml);
	var $tabedi = this;   //Read reference to the current table, to resolve "this" here.
    //Process "addButton" parameter
    if (params.$addButton != null) {
        //Se proporcionÃ³ parÃ¡metro
...
//Itera por los campos editables de una fila
    ...
    //Indica si la columna pasada estÃ¡ configurada para ser editable
        if (colsEdi==null) {  //no se definiÃ³
...
function FijModoEdit(but) {
    ...
    var $row = $(but).parents('tr');  //accede a la fila
    $row.attr('id', 'editing');  //indica que estÃ¡ en ediciÃ³n
}
...
//Acepta los cambios de la ediciÃ³n
    var $row = $(but).parents('tr');  //accede a la fila
    var $cols = $row.find('td');  //lee campos
    if (!ModoEdicion($row)) return;  //Ya estÃ¡ en ediciÃ³n
    //EstÃ¡ en ediciÃ³n. Hay que finalizar la ediciÃ³n
...
//Rechaza los cambios de la ediciÃ³n
    var $row = $(but).parents('tr');  //accede a la fila
    var $cols = $row.find('td');  //lee campos
    if (!ModoEdicion($row)) return;  //Ya estÃ¡ en ediciÃ³n
    //EstÃ¡ en ediciÃ³n. Hay que finalizar la ediciÃ³n
...
function rowEdit(but) {  //Inicia la ediciÃ³n de una fila
    var $row = $(but).parents('tr');  //accede a la fila
    var $cols = $row.find('td');  //lee campos
    if (ModoEdicion($row)) return;  //Ya estÃ¡ en ediciÃ³n
...
var $tab_en_edic = $("#" + tabId);  //Table to edit
    ...
    if ($filas.length==0) {
        ...
        //Hay otras filas, podemos clonar la Ãºltima fila, para copiar los botones
...
function TableToCSV(tabId, separator) {  //Convierte tabla a CSV
    ...
    $tab_en_edic.find('tbody tr').each(function() {
        //Termina la ediciÃ³n si es que existe
        if (ModoEdicion($(this))) {
            $(this).find('#bAcep').click();  //acepta ediciÃ³n
        }
